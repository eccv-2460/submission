...
from network import GradientReverseFunction
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, align=False):
        super().__init__()
        ...
        self.discriminator = torch.nn.Linear(256, 1, bias=True)
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1:
            if H * W != N:
                task_tokens, x = torch.split(x, [N - H * W, H * W], dim=1)
            x_ = x.permute(0, 2, 1).reshape(B, C, H, W)
            x_ = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1)
            x_ = self.norm(x_)

            if H * W != N:
                x_ = torch.cat([task_tokens, x_], dim=1)
        ...
        if self.align:
            features = GradientReverseFunction.apply(*self.mlp(k))
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.functional.gelu, norm_layer=nn.LayerNorm, sr_ratio=1, align=False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(
            dim,
            num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio, align=align)
